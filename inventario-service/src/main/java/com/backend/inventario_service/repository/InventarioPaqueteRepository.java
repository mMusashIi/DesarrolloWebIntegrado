package com.backend.inventario_service.repository;
...
public interface InventarioPaqueteRepository extends JpaRepository<InventarioPaquete, Long> {
    ...
    List<InventarioPaquete> findByIdPaquete(Long idPaquete);
    ...
    Optional<InventarioPaquete> findByIdPaqueteAndFechaSalida(Long idPaquete, LocalDate fechaSalida);

    // Buscar inventario disponible por paquete
    @Query("SELECT ip FROM InventarioPaquete ip WHERE ip.idPaquete = :idPaquete AND ip.cupoDisponible > 0 AND ip.fechaSalida >= CURRENT_DATE")
