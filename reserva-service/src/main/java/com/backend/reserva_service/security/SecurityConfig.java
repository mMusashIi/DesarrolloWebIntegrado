        ...
        http.cors(cors -> cors.configurationSource(corsConfigurationSource()))
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .access((authentication, context) -> new AuthorizationDecision(
                                hasValidInternalToken(context.getRequest().getHeader("X-Internal-Token"))
                                        || (authentication.get() != null && authentication.get().isAuthenticated())))
                        .requestMatchers(HttpMethod.PUT, "/api/reservas/*/confirmar")
                        .access((authentication, context) ->
                                new AuthorizationDecision(hasValidInternalToken(
                                        context.getRequest().getHeader("X-Internal-Token"))
                                        || isAdmin(authentication.get())))
                        .requestMatchers(HttpMethod.PUT, "/api/reservas/*/cancelar")
                        .access((authentication, context) ->
                                new AuthorizationDecision(hasValidInternalToken(
                                        context.getRequest().getHeader("X-Internal-Token"))
                                        || (authentication.get() != null && authentication.get().isAuthenticated())))
